...
case class TestFloatingPoint(var sign: Boolean, var exponent: Int, var mantissa: BigInt)(implicit
    fp: FloatingPoint
) {
  ...
  def from_double(d: Double) = {
    ...
    exponent =
      ((double_bits >> 52) & exponent_mask).toInt - ((2048 - (1 << fp.exponent_width)) >> 1)
  ...
  def poke(fp: FloatingPoint)(implicit ppt: PeekPokeTester[_]): Unit = {
    ppt.poke(fp.sign, if (sign) 1 else 0)
    ppt.poke(fp.exponent, exponent)
    ppt.poke(fp.mantissa, mantissa)
  }

  def peek(fp: FloatingPoint)(implicit ppt: PeekPokeTester[_]): Unit = {
    sign = ppt.peek(fp.sign) == 1
    exponent = ppt.peek(fp.exponent).toInt
    mantissa = ppt.peek(fp.mantissa)
  }
}
...
object TestFloatingPoint {
  ...
  def peek(signal: FloatingPoint)(implicit ppt: PeekPokeTester[_]): TestFloatingPoint = {
